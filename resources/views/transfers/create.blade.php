<x-tomato-admin-container label="{{trans('tomato-admin::global.crud.create')}} {{__('Transfer')}}">
    <x-splade-form class="flex flex-col space-y-4" action="{{route('admin.transfers.store')}}" method="post">
          ...
          <x-splade-select :label="__('Deposit id')" :placeholder="__('Deposit id')" name="deposit_id" remote-url="/admin/wallets/api" remote-root="data" option-label="uuid" option-value="id" choices/>
          <x-splade-select :label="__('Withdraw id')" :placeholder="__('Withdraw id')" name="withdraw_id" remote-url="/admin/wallets/api" remote-root="data" option-label="uuid" option-value="id" choices/>
